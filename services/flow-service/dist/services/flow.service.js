...
class FlowService {
     ...
     */
    static extractTagsFromAnswer(flow, questionId, answerValue) {
     ...
     */
    static getNextQuestionIndex(flow, currentIndex, answers) {
            ...
            for (const condition of currentQuestion.nextQuestionLogic.conditions) {
                ...
                if (matches) {
                    ...
                    if (condition.skipToQuestionId) {
                        const skipIndex = flow.questions.findIndex(q => q.id === condition.skipToQuestionId);
                        if (skipIndex !== -1) {
                            return skipIndex;
                        }
                    }
     ...
     */
    static calculateProgress(flow, answeredCount) {
